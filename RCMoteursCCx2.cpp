...
#include "Servo.h"
#include "Utils.h"
#include "RCMoteursCCx2.h"



  RCMoteursCCx2::RCMoteursCCx2(){ // constructeur par défaut 
 
  } // fin constructeur par défaut 

  RCMoteursCCx2::RCMoteursCCx2(int brocheMotDroitIn, int maxARDroit, int maxAVDroit, int brocheMotGaucheIn, int maxARGauche, int maxAVGauche, int neutreDroitIn,int neutreGaucheIn ){ // constructeur avec initialisation des broches 
  ...
  //-- utiliser this pour passer les paramètres aux variables de la classe...
	...
	this->_brocheMot[_Droit]=brocheMotDroitIn;
	this->_brocheMot[_Gauche]=brocheMotGaucheIn;
...
void RCMoteursCCx2::setDebug(boolean flagIn) {
...
void RCMoteursCCx2::impulsMot(int indiceMot,int largeurImpulsIn){

     this->_mot[indiceMot].attach(_brocheMot[indiceMot]); // attache le moteur à la broche

	this->_mot[indiceMot].writeMicroseconds(largeurImpulsIn); 
}

void RCMoteursCCx2::impulsDroit(int largeurImpulsIn){

	impulsMot(_Droit, largeurImpulsIn); 
	//this->_servo[_Droit].writeMicroseconds(largeurImpulsIn); 
}

void RCMoteursCCx2::impulsGauche(int largeurImpulsIn){
	impulsMot(_Gauche, largeurImpulsIn); 
...
//----- fonction de controle de la vitesse avant d'un moteur
void RCMoteursCCx2::motAvant(int indiceMot, int vitesseIn) {
// -- vitesse entre 10 et 100% --  
 
     int impulsMot; // variable locale 
     
      this->_mot[indiceMot].attach(_brocheMot[indiceMot]); // attache le servomoteur à la broche
     
     impulsMot=map(vitesseIn, 0, 100, _neutre[indiceMot], _maxAV[indiceMot]); // calcul largeur impulsion AV correspondante
      this->_mot[indiceMot].writeMicroseconds(impulsMot); 
     delay(10); // pause 

	if (_debug) Serial.print(F("indice =")), Serial.println(indiceMot); 
	if (_debug) Serial.print(F("broche =")), Serial.println(_brocheMot[indiceMot]); 
	if (_debug) Serial.print(F("impulsion =")), Serial.println(impulsMot); 

  
} // fin motAvant 

void RCMoteursCCx2::avantDroit(int vitesseIn) {
// -- vitesse entre 10 et 100% --  
	
	motAvant(_Droit, vitesseIn); 

} // fin 

void RCMoteursCCx2::avantGauche(int vitesseIn) {
// -- vitesse entre 10 et 100% --  
	
	motAvant(_Gauche, vitesseIn); 

} // fin 


//----- fonction de controle de la vitesse arriere d'un servomoteur
void RCMoteursCCx2::motArriere(int indiceMot, int vitesseIn) {
// -- vitesse entre 10 et 100% --  
 
     int impulsMot; // variable locale 
     
     this->_mot[indiceMot].attach(this->_brocheMot[indiceMot]); // attache le moteur à la broche
     
     impulsMot=map(vitesseIn, 0, 100, _neutre[indiceMot], _maxAR[indiceMot]); // calcul largeur impulsion AV correspondante
      this->_mot[indiceMot].writeMicroseconds(impulsMot); 

	if (_debug) Serial.print(F("indice =")), Serial.println(indiceMot); 
	if (_debug) Serial.print(F("broche =")), Serial.println(this->_brocheMot[indiceMot]); 
	if (_debug) Serial.print(F("impulsion =")), Serial.println(impulsMot); 
...
void RCMoteursCCx2::arriereDroit(int vitesseIn) {
// -- vitesse entre 10 et 100% --  
	
	motArriere(_Droit, vitesseIn); 

} // fin 

void RCMoteursCCx2::arriereGauche(int vitesseIn) {
// -- vitesse entre 10 et 100% --  
	
	motArriere(_Gauche, vitesseIn); 
...
void RCMoteursCCx2::enAvant(int vitesseIn) { // met les 2 moteurs en avant
// vitesse entre 10 et 100%
  
  motAvant(_Droit, vitesseIn); // moteur Droit
  motAvant(_Gauche, vitesseIn); // moteur Gauche
  
} // fin en avant 

void RCMoteursCCx2::enAvant(int vitesseIn, int dureeIn) { // met les 2 moteurs en avant avec durée
...
void RCMoteursCCx2::enArriere(int vitesseIn) { // met les 2 moteurs en arriere
// vitesse entre 10 et 100%
  
  motArriere(_Droit, vitesseIn); // moteur Droit
  motArriere(_Gauche, vitesseIn); // moteur Gauche
  
} // fin en arriere

void RCMoteursCCx2::enArriere(int vitesseIn, int dureeIn) { // met les 2 moteurs en arrière avec durée
// vitesse entre 10 et 100% - durée en ms
...
void RCMoteursCCx2::tourneDroite(int vitesseIn) { // tourne à droite
// vitesse entre 10 et 100%
...
void RCMoteursCCx2::tourneDroite(int vitesseIn, int dureeIn) { // tourne à droite avec durée
// vitesse entre 10 et 100% - durée en ms
...
void RCMoteursCCx2::tourneGauche(int vitesseIn) { // tourne à gauche
// vitesse entre 10 et 100%
...
void RCMoteursCCx2::tourneGauche(int vitesseIn, int dureeIn) { // tourne à gauche avec durée
// vitesse entre 10 et 100% - durée en ms
...
void RCMoteursCCx2::infosAnalyseChaine() {
...
void RCMoteursCCx2::analyseChaine(String chaineReceptionIn) { // fonction d'analyse de la chaine recue
